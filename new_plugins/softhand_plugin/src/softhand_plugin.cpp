...
void SoftHandPlugin::OnUpdateSoftSyn(const common::UpdateInfo & info)
{
    ...
    for (auto i = q_fingers.begin(); i < q_fingers.end(); i++)    // fingers
    {
      ...
      for (auto j = i->begin(); j < i->end(); j++)                      // joints
      {
        tauEl[finger_idx][joint_idx] = spring_k_each[finger_idx][joint_idx]*(fingers_syn[finger_idx][joint_idx]*qR - q_fingers[finger_idx][joint_idx]);
        joint_idx++;
      }
      finger_idx++;
    }
...
void SoftHandPlugin::OnUpdateAdaptSynTau(const common::UpdateInfo & info)
{
    ...
    for (auto i = q_fingers.begin(); i < q_fingers.end(); i++)    // fingers
    {
      ...
      for (auto j = i->begin(); j < i->end(); j++)                      // joints
      {
        tauFing[finger_idx][joint_idx] = fingers_syn[finger_idx][joint_idx]*tauM - spring_k_each[finger_idx][joint_idx]*q_fingers[finger_idx][joint_idx];
        joint_idx++;
      }
      finger_idx++;
    }
...
void SoftHandPlugin::InitParams(sdf::ElementPtr _sdf)
{
  ...
  // ------------------------------ synergy coefficent --------------------------------
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[0][0], _sdf, "syn_thumb_j1", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[0][1], _sdf, "syn_thumb_j2", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[0][2], _sdf, "syn_thumb_j3", n_param )

  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[1][0], _sdf, "syn_index_j1", 0 )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[1][1], _sdf, "syn_index_j2", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[1][2], _sdf, "syn_index_j3", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[1][3], _sdf, "syn_index_j4", n_param )

  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[2][0], _sdf, "syn_middle_j1", 0 )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[2][1], _sdf, "syn_middle_j2", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[2][2], _sdf, "syn_middle_j3", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[2][3], _sdf, "syn_middle_j4", n_param )

  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[3][0], _sdf, "syn_ring_j1", 0 )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[3][1], _sdf, "syn_ring_j2", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[3][2], _sdf, "syn_ring_j3", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[3][3], _sdf, "syn_ring_j4", n_param )

  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[4][0], _sdf, "syn_little_j1", 0 )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[4][1], _sdf, "syn_little_j2", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[4][2], _sdf, "syn_little_j3", n_param )
  INITIALIZE_PARAMETER_FROM_TAG( double, fingers_syn[4][3], _sdf, "syn_little_j4", n_param )    

  // Maximum motor torque
  INITIALIZE_PARAMETER_FROM_TAG( double, mot.tauMax, _sdf, "tauMax", 6);
